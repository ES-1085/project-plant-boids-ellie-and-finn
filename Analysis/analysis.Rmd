...
island_veg_no_na <- island_veg_all %>%
  select(location, date, gps, cover, height, common, spp_code, spp, notes, POINT_X, POINT_Y, height_class, height_class_range, percent_cover_range) %>%
  na.omit(island_veg_no_na) #%>%
  #island_veg_shape <- st_as_sf(island_veg_all, coords = c(10:11))
```
...
colors <- c("herring gull" = "#21918c", "great black-backed gull" = "#440154", "common eider" = "#fde725", "canada goose" = "#5ec962", "double-crested cormorant" = "#3b528b")
```
-
...
ggplot() +
  ...
  scale_color_manual(values = colors) #use same color pallette so species match on each plot
...
cover_color <- c("0-0.01" = "#dadaeb", "0.06-0.25" = "#bcbddc", "0.26-0.50" = "#9e9ac8", "0.51-0.75" = "#756bb1", "0.76-1.00" = "#54278f")
```
-
```{r, ggplot-map-percent-cover-on-heron}
island_veg_all %>%
  filter(location == "heron") %>%
  ggplot() +
...
```
-
```{r, ggplot-map-percent-cover-on-schoodic}
island_veg_all %>%
  filter(location == "schoodic") %>%
  ggplot() +
...
```
-
```{r, ggplot-map-percent-cover-on-shabby}
island_veg_all %>%
  filter(location == "shabby") %>%
  ggplot() +
  geom_point(aes(POINT_X, POINT_Y, color = percent_cover_range)) +
...
height_color <- c("0-0.15" = "#ffffb2", "0.15-0.30" = "#fecc5c", "0.30-1" = "#fd8d3c", "1-1.5" = "#f03b20", "1.5+" = "#bd0026")
```
-
