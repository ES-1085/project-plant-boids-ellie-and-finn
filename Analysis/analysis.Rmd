...
heron_bird_all <- heron_bird %>%
  mutate(species = "NA") %>% #create species column for heron
  select(Point_ID, GPS_Date, species, location, geometry)
...
-
```{r}
#There is no species data for Heron Island
island_bird %>%
...
```{r species-barplot-by-island, fig.height=7, dev='jpeg'}
island_veg_all %>%
  ...
  theme(axis.text.y = element_text(vjust = 10)) +
  theme_minimal() + 
  labs(title = "Vegetation disribution by island",
       x = "Number of occurences",
       y = "Common names")
